...
impl RenderSystem {
    ...
    pub fn new(renderer:renderer::Renderer) -> RenderSystem {
    ...
    pub fn render(&mut self) {
        for rendercommand in &mut self.render_commands {
            ...
            let mut model = na::Matrix4::new_translation(&na::Vector3::<f32>::new(rendercommand.position.x.round(),rendercommand.position.y.round(),0f32));
            model =
                model * na::Matrix4::from_scaled_axis(&na::Vector3::z() * rendercommand.rotation);

            model = model.prepend_nonuniform_scaling(&(rendercommand.scale.xyy()));
...
pub struct RenderCommandsSystem {
    pub m_renderer: Rc<RefCell<RenderSystem>>,
}
use specs::{Join, ReadStorage, System};

impl<'a> System<'a> for RenderCommandsSystem {
    type SystemData = (
        ReadStorage<'a, components::Transform>,
        ReadStorage<'a, components::Sprite>,
    );
    fn run(&mut self, (transform, sprite): Self::SystemData) {
        let mut localrendererclone = self.m_renderer.clone();
        let mut localrenderer = localrendererclone.borrow_mut();
        for (pos, sprite) in (&transform, &sprite).join() {
            localrenderer.render_commands.push(RenderCommand::new(
                (*pos).position,
                (*pos).rotation,
                (*pos).scale,
                (*sprite).texid,
                na::Vector4::<f32>::zeros()
            ));
        }
